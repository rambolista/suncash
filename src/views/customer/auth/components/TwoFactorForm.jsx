...
const TwoFactorForm = () => {
  ...
  const { verifyTwoFactor, loading: verifying, error: verificationError } = useCustomerAuth()
  ...
  useEffect(() => {
    ...
    ApiService.customerGetTwoFactorStatus()
      .then(setStatus)
  ...
  const handleSetup = async (event) => {
    ...
    setSubmitting(true)
    try {
      const response = await ApiService.customerSetupTwoFactor(method, currentPassword)
      setSetup(response)
      setCode(emptyCode())
      setCurrentPassword('')
      showNotification({
  ...
  const handleConfirm = async (event) => {
    ...
    setSubmitting(true)
    try {
      const response = await ApiService.customerConfirmTwoFactor(setup.challenge, code.join(''))
  ...
  const handleDisable = async (event) => {
    ...
    setSubmitting(true)
    try {
      const response = await ApiService.customerDisableTwoFactor(currentPassword)
    ...
    return (
      <Form onSubmit={handleConfirm}>
        ...
        <div className="d-grid gap-2 mt-3">
          ...
          <Button variant="outline-secondary" type="button" onClick={() => navigate('/customer/dashboard')}>
            Back to Dashboard
          </Button>
        </div>
      </Form>
