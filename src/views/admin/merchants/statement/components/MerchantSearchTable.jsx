...
const columns = [
  ...
  textCol('dba_name'),
  amountCol('client_prefund'),
  { data: 'id', orderable: false, searchable: false, width: '110px', className: 'text-nowrap action-cell', render: (id) => `<div class="statement-action-slot" data-id="${id}"></div>` },
]

const headers = ['Shortcode', 'Merchant Name', 'Balance', 'Action']

const MerchantSearchTable = ({ data, onView }) => {
  ...
  const createdRow = useMemo(() => (row, rowData) => {
    ...
    const slot = row.querySelector('.statement-action-slot')
    if (!slot) return
...
export default MerchantSearchTable
