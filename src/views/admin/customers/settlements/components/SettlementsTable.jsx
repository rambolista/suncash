...
const SettlementsTable = ({ data, tab, onView, onColumnFilterChange, searchValue, onSearchChange }) => {
  ...
  // null = not filterable (Due Date is computed, not stored; Action has no data).
  const filterKeys = useMemo(() => {
    const base = ['created_date', 'transaction_id', 'customer_name', 'channel', 'withdrawal_type', 'amount']
    ...
    {searchSlot && createPortal(
      <div style={{ minWidth: 260 }}>
        <Form.Control
          ...
          placeholder="Search transaction ID, customer name, mobile, or email"
        />
