...
const columns = [
  ...
  { data: 'channel', render: (value) => escapeHtml(value || '—') },
  { data: 'withdrawal_type', render: (value) => escapeHtml(value || '—') },
...
const headers = ['Date/Time', 'Transaction ID', 'Processed From', 'Withdrawal Type', 'Amount', 'Status']

/** Legacy's "View Settlements" button — this same customer's other settlement requests over the last 2 months. */
const SettlementHistoryModal = ({ show, onHide, settlementId, customerName }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    ApiService.getCustomerSettlementHistory(settlementId)
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Settlement History — {customerName}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {loading ? (
