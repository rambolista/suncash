...
import KycUpgradeTable from './components/KycUpgradeTable'
import KycDetailPage from './components/KycDetailPage'
...
const STATUS_BY_TAB = { pending: 'pending', approved: 'approved', rejected: 'rejected' }
...
const KycUpgradePage = () => {
  ...
  const load = () => {
    setLoading(true)
    ApiService.getKycUpgrades(tab, page, search, columnFilters)
      .then((data) => {
        setRows(data?.data || [])
      ...
      .catch((err) => showNotification({ title: 'Failed', message: err?.message || 'Failed to load KYC upgrade requests.', variant: 'danger' }))
  ...
  const handleExport = async (format) => {
    setExporting(format)
    try {
      const { blob, filename } = await ApiService.exportKycUpgrades(STATUS_BY_TAB[tab], format)
