...
const BankLoadsTable = ({ data, tab, onView }) => {
  ...
  const actionCol = {
    ...
    render: (id) => `<div class="bank-load-action-slot" data-id="${id}"></div>`,
  }
  ...
  const headers = useMemo(() => {
    const base = ['Date/Time Submitted', 'Transaction ID', 'Customer Name', 'Amount']
  ...
  const createdRow = useMemo(() => (row, rowData) => {
    ...
    const slot = row.querySelector('.bank-load-action-slot')
    if (!slot) return
