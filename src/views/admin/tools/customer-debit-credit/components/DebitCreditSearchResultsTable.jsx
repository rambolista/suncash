...
/** Legacy's single row action for this screen (`dc_btnViewDetailsCustomer`) — just "View Details". */
const DebitCreditSearchResultsTable = ({ data, onOpen }) => {
  ...
  const actionCol = {
    ...
    render: (id) => `<div class="debit-credit-action-slot" data-id="${id}"></div>`,
  }
  ...
  const createdRow = useMemo(() => (row, rowData) => {
    ...
    const slot = row.querySelector('.debit-credit-action-slot')
    if (!slot) return
    const root = slot.__actionRoot || createRoot(slot)
    slot.__actionRoot = root

    root.render(<ActionButton label="View Details" icon="eye" onClick={() => handlers.current.onOpen(item)} />)
...
export default DebitCreditSearchResultsTable
