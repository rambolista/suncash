...
const columns = [
  ...
  textCol('email'),
  {
    ...
    render: (value, type) => {
      ...
      const variant = value === 'NONE' ? 'secondary' : 'success'
      return `<span class="badge bg-${variant}-subtle text-${variant} badge-label">${escapeHtml(value)}</span>`
    },
  },
  textCol('updated_by'),
  { data: 'updated_date', render: (value, type) => (type === 'display' ? formatDateTime(value) : value || '') },
  {
    data: 'id',
    orderable: false,
    searchable: false,
    ...
    render: (id) => `<div class="kiosk-partner-action-slot" data-id="${id}"></div>`,
  },
]

const headers = ['Creation Date', 'First Name', 'Last Name', 'Mobile', 'Email', 'Payment Method', 'Last Update By', 'Last Update Date', 'Action']

const PartnersTable = ({ data, canEdit, canDelete, onEdit, onDelete }) => {
  ...
  const createdRow = useMemo(() => (row, rowData) => {
    ...
    const slot = row.querySelector('.kiosk-partner-action-slot')
    if (!slot) return
