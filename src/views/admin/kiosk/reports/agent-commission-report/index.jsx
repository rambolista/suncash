...
import AgentCommissionReportTable from './components/AgentCommissionReportTable'
...
const KioskAgentCommissionReportTab = ({ canExport = true }) => {
  ...
  const load = (filters = buildFilters()) => {
    setLoading(true)
    ApiService.getKioskAgentCommissionReport(filters)
      .then((data) => {
      ...
      .catch((err) => showNotification({ title: 'Failed', message: err?.message || 'Failed to load agent commission report.', variant: 'danger' }))
  ...
  const handleExport = async (format) => {
    setExporting(format)
    try {
      const { blob, filename } = await ApiService.exportKioskAgentCommissionReport(buildFilters(), format)
  ...
  return (
    <>
      ...
      <Card>
        <CardBody>
          {loading ? <LoadingState message="Loading agent commission report..." /> : <AgentCommissionReportTable data={rows} />}
        </CardBody>
      </Card>
    </>
  )
...
export default KioskAgentCommissionReportTab
