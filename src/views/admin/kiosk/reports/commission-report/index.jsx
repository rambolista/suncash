...
const KioskCommissionReportTab = ({ canExport = true }) => {
  ...
  const load = (filters = buildFilters()) => {
    setLoading(true)
    ApiService.getKioskCommissionReport(filters)
      .then((data) => {
      ...
      .catch((err) => showNotification({ title: 'Failed', message: err?.message || 'Failed to load commission report.', variant: 'danger' }))
  ...
  const handleExport = async (format) => {
    setExporting(format)
    try {
      const { blob, filename } = await ApiService.exportKioskCommissionReport(buildFilters(), format)
  ...
  return (
    <>
      ...
      <Card>
        <CardBody>
          {loading ? <LoadingState message="Loading commission report..." /> : <CommissionReportTable data={rows} />}
        </CardBody>
      </Card>
    </>
  )
...
export default KioskCommissionReportTab
