...
const columns = [
  ...
  textCol('location'),
  textCol('date'),
  textCol('settlement_no'),
  textCol('user'),
  textCol('previous_settlement'),
  { data: 'total_transactions', render: (value) => escapeHtml(value ?? 0) },
  {
    data: 'settlement_no',
    orderable: false,
    searchable: false,
    ...
    render: (id) => `<div class="kiosk-zout-action-slot" data-id="${id}"></div>`,
  },
]
...
const ZoutReportsTable = ({ data, onView }) => {
  ...
  const rowMap = useMemo(() => {
    const map = {}
    data.forEach((item) => { map[item.settlement_no] = item })
    return map
  }, [data])

  const createdRow = useMemo(() => (row, rowData) => {
    const item = rowMap[rowData.settlement_no] ?? rowData
    const slot = row.querySelector('.kiosk-zout-action-slot')
    if (!slot) return
...
export default ZoutReportsTable
