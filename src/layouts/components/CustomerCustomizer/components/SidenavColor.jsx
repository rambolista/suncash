...
const SidenavColor = () => {
  ...
  const sidenavColor = customerSettings.sidenavColor ?? 'dark'

  return (
    <div id="customer-sidenav-color" className="p-3 border-bottom border-dashed">
      <h5 className="mb-3 fw-bold">Sidenav Color</h5>
      <div className="row g-3">
        {sidenavColorOptions.map((option) => (
          <div className="col-4" id={`customer-sidenav-color-${option.value}`} key={option.value}>
            <div className="form-check sidebar-setting card-radio">
              <input
                className="form-check-input"
                type="radio"
                name="customer-data-menu-color"
                id={`customer-layout-sidenav-color-${option.value}`}
                checked={sidenavColor === option.value}
                onChange={() => updateCustomerSettings({ sidenavColor: option.value })}
              />
              <label className="form-check-label p-0 w-100" htmlFor={`customer-layout-sidenav-color-${option.value}`}>
