...
const SidenavSize = () => {
  ...
  const sidenavSize = customerSettings.sidenavSize ?? 'default'

  return (
    <div id="customer-sidenav-size" className="p-3 border-bottom border-dashed">
      <h5 className="mb-3 fw-bold">Sidebar Size</h5>
      <div className="row g-3">
        {sidenavSizeOptions.map((option) => (
          <div className="col-4" id={`customer-sidenav-size-${option.value}`} key={option.value}>
            <div className="form-check sidebar-setting card-radio">
              <input
                className="form-check-input"
                type="radio"
                name="customer-data-sidenav-size"
                id={`customer-layout-sidenav-size-${option.value}`}
                checked={sidenavSize === option.value}
                onChange={() => updateCustomerSettings({ sidenavSize: option.value })}
              />
              <label className="form-check-label p-0 w-100" htmlFor={`customer-layout-sidenav-size-${option.value}`}>
