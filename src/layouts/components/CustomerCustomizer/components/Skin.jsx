...
const Skin = () => {
  ...
  const skin = customerSettings.skin ?? 'default'

  return (
    <div id="customer-skin" className="p-3 border-bottom border-dashed">
      <h5 className="mb-3 fw-bold">Select Theme</h5>
      <div className="row g-3">
        {skinOptions.map((item) => (
          <div className="col-6" id={`customer-skin-${item.value}`} key={item.value}>
            <div className="form-check card-radio">
              <input
                className="form-check-input"
                type="radio"
                name="customer-data-skin"
                id={`customer-demo-skin-${item.value}`}
                checked={skin === item.value}
                onChange={() => updateCustomerSettings({ skin: item.value })}
              />
              <label className="form-check-label p-0 w-100" htmlFor={`customer-demo-skin-${item.value}`}>
